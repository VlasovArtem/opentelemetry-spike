...
import (
	...
	"os"
	"path"
	"spike-go-opentelemetry-logging/pkg/otel/exporters/otlp/otlplog/envconfig"
	"strings"
	"time"
)
	...
	DefaultEnvOptionsReader.Apply(
		envconfig.WithURL("ENDPOINT", func(u *url.URL) {
			...
			opts = append(opts, newSplitOption(func(cfg Config) Config {
				cfg.LogRecords.Endpoint = u.Host
				// For OTLP/HTTP endpoint URLs without a per-signal
				// configuration, the passed endpoint is used as a base URL
				// and the signals are sent to these paths relative to that.
				cfg.LogRecords.URLPath = path.Join(u.Path, DefaultLogRecordsPath)
				return cfg
...
func withEndpointForGRPC(u *url.URL) func(cfg Config) Config {
	return func(cfg Config) Config {
		...
		cfg.LogRecords.Endpoint = path.Join(u.Host, u.Path)
		return cfg
	}
}
